...
clear all
%% netlist path %%
%"C:\Users\gauti\Documents\MATLAB\cmi_11b_prbs.net"
%% .exe path %%
%"C:\Program Files\LTC\LTspiceXVII\scad3.exe"

%% My netlist
netlist = ['C:\Users\gauti\Documents\MATLAB\cmi_11b_prbs.net'];

input_amp = 0.7; % Input sine amp in volt
input_freq = 0.07; % fin = input_freq * fs

data = fileread('netlist_11b_prbs.txt');
txt_data2 = strsplit(data,'\n');
str = string(txt_data2);
string_to_write = ['.param X = ', num2str(input_freq), newline];
string_for_amp = ['.param vamp = ', num2str(input_amp), newline];
...
fileName = 'cmi_11b_prbs.raw';
dos('LTSpice_call_prbs11bit.bat');
pause(30) ;
...
[PRBS_mat, rnd] = construct_PRBS_mat_11b(N, CF);
...
subplot(2,1,2);
plot(f, scaling_factor*Reconstructed_spectrum);
xlabel('f');
ylabel('|X(f)|');
