...
CF = 4;
M = N/CF;
% PRBS_mat = construct_PRBS_mat(length(noisy_sig), CF);
...
set(gca, 'FontSize', 8, 'FontWeight', 'bold');
ylabel('Residual Signal', 'FontSize', 16);
xlabel('time (in sec)', 'FontSize', 16);
% xlim([1 512]);
grid on;
...
%     end
% end
RSNR_omp = snr(threshold_sig, threshold_sig - y)
...
ylabel('Residual Signal', 'FontSize', 10);
