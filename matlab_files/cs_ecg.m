...
%}
path = "C:\Users\gauti\OneDrive\Documents\MATLAB\MLII\1 NSR\100m (3).mat";
ecg_data = load(path);
% plot(t, ecg_data);
% max(ecg_data.val)
% min(ecg_data.val)
sig = ecg_data.val/1; % in mV

% [t,signal,Fs,labels]=rdmat('rec_3m');
% sig = signal(:, 2)';
% sig = sig - mean(sig);
sig = sig(1 : 2048);
N = length(sig);
...
%% Thresholding
W = 13; % Averaging window length
amp = max(sig) - min(sig);
th = 0.25 * amp; % Thresholding parameter
curr_avg = 0; % 0 for other averaging window
prev_num = 0;
threshold_sig = zeros(size(sig));
sums = zeros(size(sig));
for i = 1 : length(sig)
    if((sig(i) - mean(sig)) >= th + curr_avg) %% Ignored negative peaks, only considering R-R
        threshold_sig(i) = sig(i) - mean(sig);
    else
        threshold_sig(i) = 0;
    end
    
    if(i > 1)
        sums(i) = (sums(i - 1) - prev_num + (sig(i) - mean(sig)));
    else
        sums(i) = (curr_avg * W - prev_num + (sig(i) - mean(sig)));
    end
    
    curr_avg = sums(i)/W;
    if(i - W >= 0)
        prev_num = sig(i - W + 1) - mean(sig);
    end
end
rest_sig = sig - threshold_sig;
...
%         rest_sig(j) = sig(j);
%     else
%         rest_sig(j) = right_sum/W2;
%     end
%     
%     if(j + W2 <= length(rest_sig))
%         right_sum = right_sum - rest_sig(j + W2);
%     end
%     
%     right_sum = right_sum + rest_sig(j);
%     
%     j = j - 1;
% end
...
PRBS_mat = construct_PRBS_mat(N, M, 256, ckt_prbs);
% sameRowsPRBS = repmat(PRBS_mat(1 : 8, :), 64, 1);
...
observed_sig = round(theta * threshold_sig');
% subplot(2,2,3);
...
downsample = filtered_residue(1 : CF : end);

reconstructed = zeros(1, N);
for i = 1 : length(downsample)
    reconstructed(CF * i) = downsample(i);
end
reconstructed = CF * filtfilt(B, A, reconstructed);

subplot(3,1,2);
% f = linspace(0,fs/2 - fs/N, N/2);
plot(t,reconstructed, '-black', 'LineWidth', 1);
set(gca, 'FontSize', 5, 'FontWeight', 'bold');
title(['Residual signal after downsampling and upsampling'], "FontSize", 10, "FontWeight", "bold");
ylabel('Reconstructed residue', 'FontSize', 10);
xlabel('Time (in sec)', 'FontSize', 10);

subplot(3,1,3);
plot(t , rest_sig - reconstructed, '-black', 'LineWidth', 1);
set(gca, 'FontSize', 5, 'FontWeight', 'bold');
...
xlabel('Time (in sec)', 'FontSize', 16);
grid on;
title("Final reconstrudted signal", "FontSize", 18, "FontWeight", "bold");
